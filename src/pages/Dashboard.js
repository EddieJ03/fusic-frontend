...
const Dashboard = () => {
  ...
  const getUser = async () => {
    try {
      const response = await axios.get(
        `${process.env.REACT_APP_BACKEND_BASE}/user`,
        {
  ...
  const deleteProfile = async () => {
    setDeleting(true);
    try {
      await axios.delete(`${process.env.REACT_APP_BACKEND_BASE}/delete`, {
        withCredentials: true,
        data: {
  ...
  return cookies.AuthToken && user && user.onboarded ? (
      ...
      >
        {deleting ? (
          ...
          <>
            ...
            <p>
              This action will permenanently remove all your information
              including any messages with others.
            </p>
            <button className="secondary-button" onClick={closeModal}>
              No
            </button>
            <button className="primary-button" onClick={deleteProfile}>
              Yes
            </button>
          </>
        )}
