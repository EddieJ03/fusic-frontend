  ...
  function newMessage(message, user) {
    if (user != cookies.UserId) {
      if (clicked.current && user === clicked.current.user_id) {
        ...
        setDescendingOrderMessages((prevState) => [
          ...prevState,
          formattedMessage,
        ]);
      } else {
  ...
  const getMatches = async () => {
    ...
    try {
      const response = await axios.get(
        `${process.env.REACT_APP_BACKEND_BASE}/matched-users`,
        {
          withCredentials: true,
          params: {
            userId: cookies.UserId,
            userIds: JSON.stringify(user.matches),
          },
        }
      );
  ...
  return (
    <>
              ...
              <div
                ...
                onClick={() => {
                  ...
                  if (user.new_messages.includes(match.user_id)) {
                    axios.put(
                      `${process.env.REACT_APP_BACKEND_BASE}/update-new-message`,
                      {
                        withCredentials: true,
                        data: {
      ...
      {clickedUser ? (
        ...
        <h1 style={{ alignSelf: "center", width: "69%" }}>
          Start By Clicking a Friend on the Left!
        </h1>
      )}
    </>
  );
